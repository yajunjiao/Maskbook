...
function createContract<T extends Contract>(account: string, address: string, ABI: AbiItem[]) {
    // no a valid contract address
    if (!EthereumAddress.isValid(account)) return null
...
export function useContracts<T extends Contract>(listOfAddress: string[], ABI: AbiItem[]) {
    const account = useAccount()
    return useMemo(() => listOfAddress.map((address) => createContract<T>(account, address, ABI)), [
        account,
        listOfAddress,
        ABI,
    ])
